...
// 高阶组件的本质不是一个组件，而是一个函数，这个函数接收一个待扩展的组件(A)，然后函数内部创建一个新的组件(B)
// 组件B的模板内容是组件A(组件A被组件B包裹)，我们可以在组件B中进行功能扩充，最后返回B组件，这样就实现了在组件A
// 添加组件B提供的功能。

// 高阶组件实现逻辑复用：把需要复用的逻辑写在高阶组件中，这样高阶组件函数就可以对任何组件进行扩充。
